...
-- 1. learning_user_stats: add learner_token_id, make user_id nullable
...
-- learning_user_stats
CREATE POLICY IF NOT EXISTS "learner_stats_anon_insert" ON learning_user_stats
  FOR INSERT TO anon WITH CHECK (learner_token_id IS NOT NULL);
CREATE POLICY IF NOT EXISTS "learner_stats_anon_select" ON learning_user_stats
  FOR SELECT TO anon USING (learner_token_id IS NOT NULL);
CREATE POLICY IF NOT EXISTS "learner_stats_anon_update" ON learning_user_stats
  FOR UPDATE TO anon USING (learner_token_id IS NOT NULL) WITH CHECK (learner_token_id IS NOT NULL);

-- learning_achievements
CREATE POLICY IF NOT EXISTS "learner_achievements_anon_insert" ON learning_achievements
  FOR INSERT TO anon WITH CHECK (learner_token_id IS NOT NULL);
CREATE POLICY IF NOT EXISTS "learner_achievements_anon_select" ON learning_achievements
  FOR SELECT TO anon USING (learner_token_id IS NOT NULL);

-- learning_user_interactions
CREATE POLICY IF NOT EXISTS "learner_interactions_anon_insert" ON learning_user_interactions
  FOR INSERT TO anon WITH CHECK (learner_token_id IS NOT NULL);
CREATE POLICY IF NOT EXISTS "learner_interactions_anon_select" ON learning_user_interactions
  FOR SELECT TO anon USING (learner_token_id IS NOT NULL);
