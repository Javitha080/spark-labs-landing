...
const Header = () => {
  ...
  return (
    <>
      <ScrollProgress />
      <header
        className={`fixed left-0 right-0 z-[150] transition-all duration-500 ease-out ${scrolled
          ? "top-2 sm:top-6"
          : "top-0"
          }`}
      >
        <nav className={`mx-auto transition-all duration-500 ease-out ${scrolled
          ? "w-[92%] sm:w-[94%] max-w-7xl rounded-full h-14 sm:h-16 bg-background/60 dark:bg-background/40 backdrop-blur-xl backdrop-saturate-150 border border-black/5 dark:border-white/10 shadow-lg ring-1 ring-black/5 dark:ring-white/5"
          ...
          <div className="flex items-center justify-between h-full px-2 sm:px-4 md:px-6">
            ...
            <div className="hidden xl:flex items-center gap-1">
              ...
              <Link
                to="/blog/stem-learning-hub"
                className="nav-link relative px-4 py-2 text-sm font-medium text-foreground hover:text-primary transition-colors flex items-center gap-1"
              >
                STEM Hub <span className="flex h-2 w-2 relative"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-primary"></span></span>
              </Link>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Modern Hamburger Menu with Sheet */}
              <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
                ...
                <SheetContent side="right" className="w-full sm:w-96 p-0 border-l-primary/10 bg-gradient-to-b from-card/95 via-background/95 to-muted/95 backdrop-blur-xl">
                  <SheetHeader className="px-6 pt-8 pb-4">
                    <SheetTitle className="text-left bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">Menu</SheetTitle>
                    ...
                    <div className="flex flex-col gap-2 pb-8">
                      {menuItems.map((item, index) => (
                        <button
                          key={item.id}
                          onClick={() => scrollToSection(item.id)}
                          className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-gradient-to-r hover:from-primary/10 hover:to-secondary/5 transition-all duration-300 group border border-transparent hover:border-primary/20 animate-in slide-in-from-right-8 fade-in-0 fill-mode-backwards"
                          style={{
                      ...
                      >
                        ...
                        <div className="flex flex-col items-start">
                          <span className="text-lg font-semibold group-hover:text-primary transition-colors">STEM Hub</span>
                        </div>
                        <Zap className="w-4 h-4 ml-auto text-primary/0 group-hover:text-primary/60 transition-all group-hover:translate-x-1" />
                      </Link>

                      <div className="pt-4 animate-in slide-in-from-right-8 fade-in-0 fill-mode-backwards" style={{ animationDelay: `${(menuItems.length + 1) * 50}ms` }}>
                        <Button
                          onClick={() => {
                      ...
                      {/* Social Links */}
                      <div className="mt-4 pt-6 border-t border-primary/10 animate-in slide-in-from-right-8 fade-in-0 fill-mode-backwards" style={{ animationDelay: `${(menuItems.length + 2) * 50}ms` }}>
