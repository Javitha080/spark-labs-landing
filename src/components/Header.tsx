  ...
  return (
    <>
      ...
      >
        <nav className="container mx-auto h-full px-4 md:px-6">
          <div className="flex items-center justify-between h-full">
            {/* Logo */}
            <div
              className={`flex items-center gap-3 transition-all duration-300 ${scrolled ? "scale-90" : "scale-100"
                }`}
            >
              <div className="relative group">
                ...
                <div className="relative w-10 h-10 md:w-12 md:h-12 rounded-xl overflow-hidden shadow-lg ring-2 ring-primary/20 group-hover:ring-primary/40 transition-all">
                  <img
                    src={clubLogo}
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <div className="relative w-6 h-6">
                  <span
                    className={`absolute left-0 top-1 w-6 h-0.5 bg-foreground transition-all duration-300 ${isMenuOpen ? "rotate-45 top-2.5" : ""
                      }`}
                  />
                  <span
                    className={`absolute left-0 top-2.5 w-6 h-0.5 bg-foreground transition-all duration-300 ${isMenuOpen ? "opacity-0" : ""
                      }`}
                  />
                  <span
                    className={`absolute left-0 top-4 w-6 h-0.5 bg-foreground transition-all duration-300 ${isMenuOpen ? "-rotate-45 top-2.5" : ""
                      }`}
                  />
                </div>
              </button>
      ...
      >
        {/* Backdrop */}
        <div
          className={`absolute inset-0 backdrop-blur-xl bg-background/95 transition-opacity duration-300 ${isMenuOpen ? "opacity-100" : "opacity-0"
            }`}
        ...
        {/* Menu Content */}
        <div
          className={`absolute right-0 top-0 h-full w-full sm:w-80 bg-gradient-to-b from-background to-muted/30 shadow-2xl transition-transform duration-300 ${isMenuOpen ? "translate-x-0" : "translate-x-full"
            }`}
        >
            ...
            <nav className="flex-1 space-y-2">
              {menuItems.map((item, index) => (
                <button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className={`mobile-menu-item w-full flex items-center gap-4 p-4 rounded-xl hover:bg-primary/10 transition-all group ${isMenuOpen ? "animate-slide-in-right" : ""
                    }`}
